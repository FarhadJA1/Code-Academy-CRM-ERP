  ...
  return (
    <div>
      {(props.search.length===0)?
        (props.groups.length>0)?<table className="table table-hover">
        <thead>
          <tr>
        ...
        <tbody>
          {props.groups.slice((props.currentPage-1)*props.pageLimit,props.currentPage*props.pageLimit).map(group => (
            <tr key={group.id}>
      ...
      <tbody>
        {props.search?.slice((props.currentPage-1)*props.pageLimit,props.currentPage*props.pageLimit).map(group => (
          <tr key={group.id}>
            <th className='table-header' align='center' scope="row">{count++}</th>
            <td className='table-header' align='center'>{group.groupCode}</td>
            <td className='table-header' align='center'>{group.createDate.substring(0, 10).split("-").reverse().join("-")}</td>
