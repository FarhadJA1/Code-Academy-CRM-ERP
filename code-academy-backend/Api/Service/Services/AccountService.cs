...
namespace Service.Services
{
    public class AccountService:IAccountService
    {
        ...
        public async Task UpdatePassword(AppUser appUser, UpdatePasswordDto updatePasswordDto)
        {
            ...
            var result = await _userManager.ChangePasswordAsync(appuser, updatePasswordDto.CurrentPassword, updatePasswordDto.NewPassword);
            var user = _mapper.Map(updatePasswordDto, appuser);
            var upuser = await _userManager.UpdateAsync(user);

        }
        public async Task ChangePhoto(AppUser appUser, ChangePhotoDto changePhotoDto)
        {
            var appuse = await _userManager.FindByIdAsync(appUser.Id);
            var user = _mapper.Map(changePhotoDto, appuse);
            var upuser = await _userManager.UpdateAsync(user);
        }
